...
/* Guard: require guid detection so backup is restorable */
SET @go_can_mutate_map := IF(@allow_apply = 1 AND @go_has_map = 1 AND @go_has_guid = 1, 1, 0);

SET @sql := IF(@go_can_mutate_map = 1,
...
SET @deleted_invalid_map := IF(@go_can_mutate_map = 1, ROW_COUNT(), 0);
DEALLOCATE PREPARE stmt;

/* ── B: Backup + update invalid phase rows ───────────────────────── */
/* Guard: require guid detection so backup is restorable */
SET @go_can_mutate_phase := IF(@allow_apply = 1 AND @go_has_phase = 1 AND @go_has_guid = 1, 1, 0);

SET @sql := IF(@go_can_mutate_phase = 1,
...
SET @sql := IF(@go_can_mutate_phase = 1,
  ...
  'SELECT 0');
PREPARE stmt FROM @sql; EXECUTE stmt;
SET @updated_phase := IF(@go_can_mutate_phase = 1, ROW_COUNT(), 0);
DEALLOCATE PREPARE stmt;

/* ── C: Backup + update invalid model rows ───────────────────────── */
/* Guard: require guid detection so backup is restorable */
SET @cr_can_mutate_model := IF(@allow_apply = 1 AND @cr_has_model = 1 AND @cr_has_guid = 1, 1, 0);

SET @sql := IF(@cr_can_mutate_model = 1,
...
SET @sql := IF(@cr_can_mutate_model = 1,
  ...
  'SELECT 0');
PREPARE stmt FROM @sql; EXECUTE stmt;
SET @updated_model := IF(@cr_can_mutate_model = 1, ROW_COUNT(), 0);
DEALLOCATE PREPARE stmt;
